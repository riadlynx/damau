...
#Mesures Corrigé
        ...
        for i in range(len(Z_)):
            MC.append(
                ggg[9][i] - Z_[i] * coe[0][1] - Z2_[i] * coe[0][2] - Z3_[i] * coe[0][3] - Z4_[i] * coe[0][4] - cos_[i] *
                coe[0][5] - sin_[i] * coe[0][6] - cos2_[i] * coe[0][7] - sin2_[i] * coe[0][8])


        ax1.plot(x, T1, color='lime', label="Extrapolation de la dérive", linewidth=2, )
        ax1.plot(x[ggg[2]:ggg[3] + 2], T2, color='g', label="Dérive sur la période", linewidth=4)
        ax1.plot(x, MC, color='darkred', label="Mesures Corrigées",marker='o',linestyle='' )
